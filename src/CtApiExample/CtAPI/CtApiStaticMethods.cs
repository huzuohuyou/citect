    ...
    public class CtApiStaticMethods
    {
        ...
        public static void ThrowLastCtapiError(string functionName)
        {
            ...
            if (IsCitectError(error))
            {
                ...
                Debug.WriteLine(String.Format("{0} failed giving citect error: {1}.", functionName, citectScadaError));
            }
            Debug.WriteLine(String.Format("{0} failed giving win32 error: {1}.", functionName, error));
        }
        #endregion
    }
}
